...
#
# I01: Verify `pio platform install` from local checkout succeeds and
...
echo
echo "=== I01 step 1: pio platform install (from local checkout) ==="
...
if [ ! -d ~/.platformio/platforms/realtek-ameba ]; then
    echo "❌ I01: ~/.platformio/platforms/realtek-ameba not created"
    exit 1
fi
...
for f in platform.json platform.py builder/main.py; do
    if [ ! -f ~/.platformio/platforms/realtek-ameba/"$f" ]; then
        echo "❌ I01: $f missing in installed platform"
        exit 1
    fi
done
...
echo
echo "=== I01 step 2: minimal project setup ==="
mkdir -p "$PROJ/src"
...
echo
echo "=== I01 step 3: first pio run (triggers SDK clone + venv + auto-skeleton) ==="
...
if ! pio run -e "$TEST_BOARD" > "$LOG" 2>&1; then
    echo "❌ I01: pio run failed. Last 40 lines of log:"
    tail -40 "$LOG"
    exit 1
fi
...
echo
echo "=== I01 step 4: validate SDK + venv + artifacts ==="

# SDK must be cloned
if [ ! -f ~/.platformio/packages/framework-ameba-rtos/ameba.py ]; then
    echo "❌ I01: SDK not cloned (no ameba.py in framework-ameba-rtos/)"
    exit 1
fi
echo "  ✓ SDK cloned"
...
if [ ! -f "$STAMP" ]; then
    echo "❌ I01: venv stamp missing — auto-resync didn't run"
    exit 1
fi
...
for path in src/main.c app_example/app_main.c app_example/CMakeLists.txt CMakeLists.txt; do
    if [ ! -f "$PROJ/$path" ]; then
        echo "❌ I01: auto-skeleton missing $path"
        exit 1
    fi
done
...
if [ ! -f "$PROJ/compile_commands.json" ]; then
    echo "❌ I01: compile_commands.json not exported to project root"
    exit 1
fi
...
if ! grep -q "user_main()" "$PROJ/src/main.c"; then
    echo "❌ I01: src/main.c starter doesn't mention user_main"
    exit 1
fi
if ! grep -q "its own" "$PROJ/src/main.c"; then
    echo "❌ I01: src/main.c starter missing 'runs on its own task' user_main hint"
    exit 1
fi
echo "  ✓ src/main.c starter has user_main hint"

echo
echo "✅ I01 install: PASS ($TEST_BOARD)"
